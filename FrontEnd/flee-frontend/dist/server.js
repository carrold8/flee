...
app.prepare().then(() => {
    ...
    io.on("connection", async (socket) => {
        ...
        const selectTiles = async (room) => {
            ...
            const isSelected = (tile) => usersSelected.some(selected => selected.x === tile.x && selected.y === tile.y);
        ...
        const handleHitUser = async (room) => {
            ...
            const users = await usersCollection
                ...
                .sort({ lives: -1 })
                .toArray();
            const distance = (userX, userY, x, y) => {
        ...
        const handleEndOfRound = async (room) => {
